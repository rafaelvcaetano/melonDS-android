...
abstract class RetroAchievementsDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    protected abstract suspend fun insertGameSets(sets: List<RAAchievementSetEntity>)

    @Query("DELETE FROM ra_achievement_set WHERE game_id = :gameId")
    protected abstract suspend fun deleteGameSets(gameId: Long)
    ...
    @Transaction
    open suspend fun updateGameData(
        gameEntity: RAGameEntity,
        sets: List<RAAchievementSetEntity>,
        achievements: List<RAAchievementEntity>,
        leaderboards: List<RALeaderboardEntity>,
    ) {
        deleteGameAchievements(gameEntity.gameId)
        deleteGameLeaderboards(gameEntity.gameId)
        deleteGameSets(gameEntity.gameId)
    ...
    @Query("SELECT * FROM ra_achievement_set WHERE id = :setId")
    abstract suspend fun getAchievementSet(setId: Long): RAAchievementSetEntity?

    @Transaction
    @Query("SELECT * FROM ra_game WHERE game_id = :gameId")
    abstract suspend fun getGameWithSets(gameId: Long): RAGameWithSetsEntity?
